 ...
 * <p>
 * Depending on your message arguments, memory may be allocated for boxing and varargs array creation. However, the
 * methods of this class have a large number of overloads that prevent such allocations in many common cases.
 * <p>
 ...
 * stack traces. See <a href="https://github.com/google/guava/wiki/ConditionalFailuresExplained">Conditional failures
 * explained</a> in the Guava User Guide for more advice. Notably, {@link Verify} offers assertions similar to those in
 * this class for non-precondition checks.
 ...
 * <p>
 * {@code Preconditions} uses {@link Strings#lenientFormat} to format error message template strings. This only supports
 * the {@code "%s"} specifier, not the full range of {@link java.util.Formatter} specifiers. However, note that if the
 * number of arguments does not match the number of occurrences of {@code "%s"} in the format string,
 * {@code Preconditions} will still behave as expected, and will still include all argument values in the error message;
 * the message will simply not be formatted exactly as intended.
 * <h3>More information</h3>
 * <p>
 * See the Guava User Guide on <a href="https://github.com/google/guava/wiki/PreconditionsExplained">using {@code
 * Preconditions}</a>.
 *
