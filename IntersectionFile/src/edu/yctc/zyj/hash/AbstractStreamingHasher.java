...
abstract class AbstractStreamingHasher extends AbstractHasher {
     ...
     *
     * @param chunkSize the number of bytes available per {@link #process(ByteBuffer)} invocation; must be at least 4
     */
     ...
     * internal buffer of {@code bufferSize} size, which must be a multiple of {@code
     * chunkSize}.
     *
     * @param chunkSize the number of bytes available per {@link #process(ByteBuffer)} invocation; must be at least 4
    ...
    /**
     * This is invoked for the last bytes of the input, which are not enough to fill a whole chunk. The passed
     * {@code ByteBuffer} is guaranteed to be non-empty.
     * <p>
    ...
    /*
     * Note: hashString(CharSequence, Charset) is intentionally not overridden. While intuitively, using CharsetEncoder
     * to encode the CharSequence directly to the buffer (or even to an intermediate buffer) should be considerably more
     * efficient than potentially copying the CharSequence to a String and then calling getBytes(Charset) on that
     * String, in reality there are optimizations that make the getBytes(Charset) approach considerably faster, at least
     * for commonly used charsets like UTF-8.
     */
